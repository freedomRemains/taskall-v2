...
## 依頼者傾向への対応方針（issue #75）

サインアップ・トップ画面等の具体的コンテンツ追加が本格化するにつれ、実装水準の詳細に
詳しくない依頼者から、大まかなUI・挙動の説明のみで改修依頼を受けることが多くなる
（DBモデルやAPI設計まで細かく指定される従来のissueの方が例外になっていく）。以下は、
そうした依頼における統計的傾向（曖昧な回答の繰り返し、同一機能への行きつ戻りつ、
文脈と無関係な一言が実は新規要望である等）への対応方針である。

- **不明点は必ず質問する。** 曖昧な回答が繰り返される場合、自由回答の質問を繰り返す
  のではなく、`ask_user`の選択肢提示（choices）を優先する。選べない場合でも消去法で
  少しずつ仕様を確定できるよう、質問ごとに「こうかな？」という補足を添える。
- **蒸し返し検知を徹底する。** 新規依頼に着手する前に、`documents/rules/1000002_issue_points.md`
  を関連キーワードで確認し、同一・類似機能について過去にOK/NGとした経緯がないか確認する。
  経緯が見つかった場合は「issue #Xで一度〜と決定していますが、それを覆す認識でよいか」と
  先回りで確認してから着手する（無自覚な蒸し返しによる手戻りを防ぐため）。
- **文脈と無関係な一言への対応。** 現在の作業文脈と無関係な一言（新機能のイメージ等)を
  受けた場合、ノイズと本命の要望を取り違えないよう、実装前に必ず「これは現在の作業とは
  別の新規依頼という認識でよいか」を確認する。
- **部品化・共通化にこだわりすぎない。** 依頼ごとに新しい画面部品（HTML_PARTS）・
  業務ロジック（SCR/Serviceクラス）を機械的な連番で作成してよい。無理に既存部品の
  再利用・共通化を優先せず、まずは指示された通りに機能を追加/変更する。ただし、同一・
  類似の依頼が複数回行き来し複雑化している場合は、共通ロジックへの集約を検討する。
  古い画面部品・業務ロジックは、復活の見込みが無いと判断できる場合（目安として、対象
  機能に3ヶ月程度変更依頼が無い場合）に`@Deprecated`を付与し、削除候補として提示する。
- **コスト認識の共有。** 着手前に「1画面・軽微な変更」等、ざっくりした規模感を一言
  伝えてから作業する。本プロジェクトは軽微な修正でも完全AI駆動で対応する方針のため、
  実際にかかるコスト（AIの利用料金・時間）と依頼者の想定コストに乖離が生じやすい。
  コストは依頼に対して使用したAIリソースの従量制で精算する。

...
- **本番反映後のトラブルとFlywayマイグレーション追加**: develop→mainマージ後、
  「案件情報」リンクが本番で表示されない不具合が発生。原因は`db/data`変更(新規
  `ANKEN`/`ATTR_IN_ANKEN`テーブル、`ATTR_GRP`/`ATTR`マスタデータ追加、9個の共有
  テーブルへの追加・更新)に対応する`db/flyway/V{n}__*.sql`マイグレーションを
  一度も作成していなかったこと(Terraform apply漏れではなく、Flywayマイグレーション
  漏れが原因だった)。`V6__add_anken_list.sql`を新規作成し、旧DB状態からの
  適用結果と新規ブートストラップ結果が全13主要テーブルで完全一致することを検証して

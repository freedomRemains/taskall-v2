...
- **AWSアカウントの契約・ルートアカウントログイン**
  - Terraformを含むあらゆるAWS操作の大前提であり、AIが人の手続きを代行する仕組みが未成熟な現状ではコード化はできず、手作業の必要がある。(※1)
...
※1  
クロスサイトスクリプティングやダークパターン回避を目的として、AIがWebの手続きを代行するようになる可能性は十分想定できる。
本資料にて手続き全体の概観を記載した理由はそこにある。再現性のある情報を、分かりやすく提示する必要がある。
(人に任せていると悪意のあるサイトにアクセスした状態のブラウザの別タブでAWS契約を行う、といったリスクすら存在し、
かつWeb側も登録したクレカ情報を解約できないようにするなどの悪質なダークパターンを仕掛けてくることが考えられる。
クリーンな環境で、人の承認を得て、安全な電子手続きの手順を事前に調べ、手続きそのものを代行することは未来像として
十分に想定しうるものである)  
背後にある複雑な仕組みを理解せずとも便利なサービスを安全に使えることは、高度に発達した社会に必須の条件である。
ハイブリッド車の全ての仕組みを詳細に理解していなければ車を運転できない規則とすれば、運転できる人自体がいなくなってしまう。
AWS契約もRoute53によるドメイン取得も、ユーザの要望を聞いて電子手続きを行えるようにするというやり方でも、
本来は問題がない。本人認証を行って手続きを代行する仕組み自体も、今現在の技術だけでも十分実現は可能である。  
現状はおそらく過渡期であると考えられる。将来的には即時にサービスは提供されるが、詳細を知りたくなったらナレッジ
ベースのようなものにアクセスして何でも知りたいことを照会できるようになる、といった仕組みが想定できる。
AIコーチングにより、契約のメリット・デメリットについて分かりやすい説明を受けて、意思表示のみを行う、
といったことも可能性が高い未来像として考えられる。(それまでは全体概略が分かる手順がないと再現性は確保できない)
